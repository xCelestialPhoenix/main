...
import static seedu.nova.logic.commands.CommandTestUtil.TIME_DATE;
...
import static seedu.nova.logic.commands.CommandTestUtil.VALID_TIME_DATE;
import static seedu.nova.logic.commands.CommandTestUtil.VALID_VENUE;
import static seedu.nova.logic.commands.CommandTestUtil.VENUE;
...
public class EventAddConsultationCommandParserTest {
    ...
    public void parse_allFieldsPresent_success() {
        ...
        assertParseSuccess(parser, PREAMBLE_WHITESPACE + CONSULTATION_DESC + VENUE + TIME_DATE,
                new EventAddConsultationCommand(expectedConsultation));

        // multiple descriptions - last description accepted
        assertParseSuccess(parser, CONSULTATION_DESC_2 + CONSULTATION_DESC + VENUE + TIME_DATE,
                new EventAddConsultationCommand(expectedConsultation));

        // multiple descriptions - last description accepted
        assertParseSuccess(parser, CONSULTATION_DESC + VENUE_2 + VENUE + TIME_DATE,
                new EventAddConsultationCommand(expectedConsultation));

        // multiple date time - last date time accepted
        assertParseSuccess(parser, CONSULTATION_DESC + VENUE + TIME_DATE_2 + TIME_DATE,
                new EventAddConsultationCommand(expectedConsultation));
    }
    ...
    public void parse_compulsoryFieldMissing_failure() {
        ...
        // missing desc prefix
        assertParseFailure(parser, VALID_CONSULTATION_DESC + VENUE + TIME_DATE, expectedMessage);

        // missing venue prefix
        assertParseFailure(parser, CONSULTATION_DESC + VALID_VENUE + TIME_DATE, expectedMessage);

        // missing time prefix
        assertParseFailure(parser, CONSULTATION_DESC + VENUE + VALID_TIME_DATE, expectedMessage);

        // missing all prefixes
        assertParseFailure(parser, VALID_CONSULTATION_DESC + VALID_VENUE + VALID_TIME_DATE, expectedMessage);
    ...
    public void parse_invalidValue_failure() {
        ...
        assertParseFailure(parser, CONSULTATION_DESC + VENUE + TIME_DATE, expectedMessage);

        // invalid time - end time is missing
        assertParseFailure(parser, CONSULTATION_DESC + VENUE + TIME_DATE, expectedMessage);

        // invalid date - date is missing
        assertParseFailure(parser, CONSULTATION_DESC + VENUE + INVALID_TIME_DATE_4, expectedMessage);
