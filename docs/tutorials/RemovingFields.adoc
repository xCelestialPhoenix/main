...
The `nova` field in `Person` is actually an instance of the `seedu.nova.model.common.person.Address` class.
...
While keeping the `nova` field in the json files does not cause the tests to fail, it is not good practice to let cruft from old features accumulate.
...
```
You can go through each individual `json` file and manually remove the `nova` field.
