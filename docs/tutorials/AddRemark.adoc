...
Create a `RemarkCommandParser` class in the `seedu.nova.logic.parser` package.
...
Create a new `Remark` in `seedu.nova.model.common.person`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
