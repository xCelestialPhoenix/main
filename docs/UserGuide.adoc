...
* Optional fields are `n\[name]`, `p\[phone number]`, `e\[email address]` or `c\[classmate/teammate]`
...
[NOTE]
====
* Adds profile picture of the person at the specified `[index]`
* The index refers to the index number shown in the displayed person list
* The index **must be a positive integer** 1, 2, 3, …​
...
==== Create Routine Task: `task`
Create routine tasks in your study plan.
...
==== Create Flexible Day-Task: `task`
Create flexible tasks in your study plan, which will create one event per day when being scheduled.
...
* Situations where the event will not be created and you will be notified:
